...
struct FVector2
{
	 ...
	 * @brief FVector를 Param으로 넘기는 생성자
	 */
	FVector2(const FVector& InOther);
	 ...
	 * @brief 두 벡터를 내적하여 결과의 스칼라 값을 반환하는 함수
	 */
	inline float Dot(const FVector2& OtherVector) const { return (X * OtherVector.X) + (Y * OtherVector.Y); }

	/**
	 * @brief 두 벡터의 외적 연산 (2D에서는 스칼라 값 반환)
	 * @return Z축 방향의 외적 크기
	 */
	inline float Cross(const FVector2& OtherVector) const
	{
	...
	inline FVector2 GetNormalized() const
	{
		...
		if (VectorLength > 0.00000001f)
		{
			return FVector2(X / VectorLength, Y / VectorLength);
		}
	 ...
	 */
	inline float Distance(const FVector2& OtherVector) const
	{
	 ...
	 */
	inline float DistanceSquared(const FVector2& OtherVector) const
	{
	 ...
	 * @brief 벡터 동등성 비교 연산자
	 */
	bool operator==(const FVector2& OtherVector) const
	{
	 ...
	 */
	inline static FVector2 Lerp(const FVector2& A, const FVector2& B, float T)
	{
		return A + (B - A) * T;
	}
